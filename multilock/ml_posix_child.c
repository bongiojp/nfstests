      ...
      if(errno == EAGAIN)
      ...
      else
        {
          ...
          sprintf(badtoken, "%s %lld %lld",
                  str_lock_type(lock.l_type), resp->r_start, resp->r_length);
          resp->r_status = STATUS_ERRNO;
          resp->r_errno  = errno;
        }
    }
  ...
  if(rc == -1)
    {
      ...
      sprintf(badtoken, "%s %lld %lld", str_lock_type(lock.l_type), resp->r_start, resp->r_length);
      resp->r_status = STATUS_ERRNO;
      return;
    }
...
void do_list(response_t * resp)
{
  ...
  while(tl_head != NULL)
    {
      conflict |= list_locks(tl_head->tl_start, tl_head->tl_end, resp);
  ...
  while((c = getopt(argc, argv, options)) != EOF)
    {
      switch (c)
        ...
        case 'c':
          ...
          if(rc == -1)
            {
              fprintf(stderr, "Can not change dir to %s errno = %d \"%s\"\n",
        ...
        case 'x':
          if((oflags & 7) != 0)
            show_usage(1, "Can not combine -x and -s/-p/-n\n");
      ...
      else
        {
          ...
          if(rest == NULL)
          ...
          else
            {
              ...
              if(*rest != '\0' && *rest != '#')
                fprintf_stderr("Command line not consumed, rest=\"%s\"\n", rest);

              switch(resp.r_cmd)
                {
                  case CMD_OPEN:    do_open(&resp);   break;
                  case CMD_CLOSE:   do_close(&resp);  break;
                  case CMD_LOCKW:   do_lock(&resp);   break;
                  case CMD_LOCK:    do_lock(&resp);   break;
                  case CMD_UNLOCK:  do_unlock(&resp); break;
                  case CMD_TEST:    do_test(&resp);   break;
                  case CMD_LIST:    do_list(&resp);   break;
                  case CMD_HOP:     do_hop(&resp);    break;
                  case CMD_UNHOP:   do_unhop(&resp);  break;
                  case CMD_SEEK:    do_seek(&resp);   break;
                  case CMD_READ:    do_read(&resp);   break;
                  case CMD_WRITE:   do_write(&resp);  break;
                  case CMD_ALARM:   do_alarm(&resp);  break;
