...
int main(int argc, char **argv) {
  ...
  if (argc < 7) {
    ...
    printf("Usage: %s filename [SHARED|EXCLUSIVE] [NOBLOCK|BLOCK|TEST] start_byte end_byte [SLEEP|NOSLEEP]\n", argv[0]);
    exit(1);
  }
  ...
  if (fl.l_type == F_UNLCK){
    printf("TEST LOCK: Operation would be allowed.\n");
    return 0;
  }
  else {
    printf("TEST LOCK: Found conflict from pid %d\n", fl.l_pid);
    return 0;
  }
