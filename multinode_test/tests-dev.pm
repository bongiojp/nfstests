...
sub test_unittest_3(\@\@$$) {
    ...
    foreach my $detectnode(@mountdirs) {
        foreach my $changenode(@mountdirs) {
            ...
            print "\tTesting by making changes on node ${_}.\n";
            if (! comm_retry("rm -rf ${detectnode}/${testdir}", $FALSE)) {
                print "WARNING: Could not delete directory ${detectnode}/${testdir}.";
            }
            ...
            foreach(1 .. $iter) {
                ...
                if ($result =~ /\d+\s+\d+\s+.+\s+\d+\s+.+\s.+\s(\d+)\s\w\w\w\s.*${testfile}.*/) {
                    my $remote_filesize = $1;
                    
                    # Get size of /etc/hosts file
                    my $result = `ls -sail /etc/hosts`;
                    if ($result =~ /\d+\s+\d+\s+.+\s+\d+\s+.+\s.+\s(\d+)\s\w\w\w\s.*hosts.*/) {
                        my $local_filesize = $1;
    ...
    foreach my $detectnode(@mountdirs) {
        foreach my $changenode(@mountdirs) {
            ...
            foreach(1 .. $iter) {
                ...
                # Test to see if file appears
                if (! ($result =~ /${testfile_bits}\s${testlink}.*/)) {
                    print "ls -sail results: ${result}\n";
